{
  description = "Custom Generators — a Civitai App Block. Dev shell: node + pnpm, pinned.";
      ...
      devShells = forAllSystems (pkgs:
        let tc = toolchain pkgs; in
        {
          default = pkgs.mkShell {
            ...
            shellHook = ''
              echo "custom-generators: node $(node --version), pnpm $(pnpm --version)"
              echo "  pnpm install --frozen-lockfile   restore node_modules"
              echo "  pnpm run dev:harness             mock host on :5188"
              echo "  pnpm typecheck && pnpm test && pnpm build   the gates CI runs"
            '';
          };
        });
