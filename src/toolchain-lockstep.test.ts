 ...
 *
 * That is not hypothetical here. Before the flake landed, this repo had no
 * flake at all and the workflow hardcoded `node-version: 22` with `cache: npm`,
 * so the only statement of the toolchain lived in a file nobody runs locally.
 * The same batch moved the repo from npm to pnpm, which means the workflow now
 * carries a SECOND pin (`pnpm/action-setup`'s `version:`) that a local shell
 * would otherwise be free to disagree with.
 *
 ...
 *
 * Read off disk rather than imported, for the same reason the release-version
 * guard in `manifest.test.ts` does it: it asserts against the bytes that
 * actually ship, and `import.meta.url` makes the paths independent of the
 * runner's working directory. (This file lives in the `node` vitest project —
 * `src/**\/*.test.ts` — so it runs without a DOM.)
 *
 ...
 * adding one to read a 37-line workflow would be a bigger change than the
...
describe('toolchain lockstep', () => {
  ...
  it('has CI read .nvmrc rather than restating the node version', () => {
    ...
    // else in reality — which is exactly the `node-version: 22` this repo
    // shipped before the flake landed.
  ...
  it('keeps the platform builder on the same package manager as CI', () => {
    ...
    // that state, and `civitai app validate` was what caught it. This repo is
    // one npm-to-pnpm migration away from having been in it.
